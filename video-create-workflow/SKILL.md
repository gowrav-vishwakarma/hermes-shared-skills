...
3. **Default: No hard cuts in I2V** -- never write `CUT TO`, `JUMP CUT`, `MEANWHILE` in per-post reels by default. Evolve the frame with camera moves (dolly, pan, tilt, push-in, pull-back).
3b. **Exception: CUT TO: when user explicitly requests it** -- If the user explicitly asks for hard cuts, meme-style formatting, or multi-scene shots with distinct close-ups, use `CUT TO:` to create hard scene transitions. Each `CUT TO:` starts a new scene with its own composition description. The opening tableau sentence must still match the anchor composition. LTX-2.3 will render the hard cut as a video frame transition. (Session 2026-05-07: "Which One Are You?" reel used CUT TO: successfully for a 2-character dialogue reel.)
