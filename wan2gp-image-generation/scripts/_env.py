...
Stdlib-only. Hermes maps ``$HOME`` to the active profile home, so
``Path.expanduser()`` and ``~/.hermes/...`` silently land on phantom nested
paths — use ``resolve_path()`` for CLI path arguments.
